...
- Added synthetic pipeline coverage for coexistence cache population and compatibility fallback behavior when product-read-model cache data is missing or stale.
...
- Promoted Dashboard, AssetTable-visible Dashboard rows and Reports from guarded safe-default coexistence to the first canonical Global Asset PRM safe-field route group: identity/display/portfolio labels are PRM-canonical when freshness/scope evidence is valid, while valuation/performance/cost-basis/dividend/quantity-sensitive fields, missing/stale/scope-mismatch/invalid PRM fallback and explicit rollback stay compatibility-backed.
- Dashboard explicit-load/cache-hydration flow now applies Global Asset PRM safe-field source selection in real app flow while preserving compatibility fallback and rollback via env flag.
- Reports canonical safe-field selection now consumes real coexistence cache data populated by the explicit assets load flow and keeps compatibility fallback for stale/scope-mismatch/missing/invalid product data.
- Dashboard and AssetTable helper layer now includes canonical safe-field selectors that consume Global Asset product rows when enabled and ready, while preserving compatibility fallback behavior for blocked or unsupported metrics.
- Reports read-model helper now uses canonical safe-field selection and compatibility-safe fallback/block behavior for unsupported valuation/performance metrics.
- Safe non-valuation Global Asset fields are now canonical for Dashboard, AssetTable and Reports when product-read-model freshness/scope evidence is ready, while explicit rollback (`NEXT_PUBLIC_GLOBAL_ASSET_PRODUCT_GUARD_ENABLED=off|false|0`) forces full compatibility.
- Dashboard/AssetTable/Reports compatibility mapping now keeps valuation/performance and transfer-sensitive fields (`positionValue`, `remainingCostBasis`, `avgBuyPrice`, `unrealizedPnL`, dividend totals, quantity-sensitive paths) compatibility-backed even when Global Asset identity/display/portfolio labels are selected.
