...
- Dashboard explicit-load/cache-hydration flow now applies guarded Global Asset source selection in real app flow while preserving compatibility-default behavior and rollback via env flag.
- Reports guarded source selection now consumes real coexistence cache data populated by the explicit assets load flow and keeps compatibility fallback for stale/scope-mismatch/missing product data.
...
- Safe non-valuation Global Asset defaults are now enabled for Dashboard, AssetTable and Reports when product-read-model freshness/scope evidence is ready, while explicit rollback (`NEXT_PUBLIC_GLOBAL_ASSET_PRODUCT_GUARD_ENABLED=off|false|0`) forces full compatibility.
- Dashboard/AssetTable/Reports guarded compatibility mapping now keeps valuation/performance and transfer-sensitive fields (`positionValue`, `remainingCostBasis`, `avgBuyPrice`, `unrealizedPnL`, quantity-sensitive paths) compatibility-backed even when Global Asset identity/display/portfolio-breakdown fields are selected.
