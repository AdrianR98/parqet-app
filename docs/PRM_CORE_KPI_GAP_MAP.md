...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Portfolio Value | `portfolio_value` | Current market value of all active positions in scope. | `src/lib/calculations/global-asset-metrics.ts`; `src/lib/parqet/global-assets/aggregate.ts`; `src/lib/calculations/view-model-aggregates.ts`; `src/app/(app)/dashboard/page.tsx`; `src/lib/reporting.ts` | `partial` | Dashboard Hero, Reports | `calculations` | Aggregation/PRM now receive an explicit DB market-price overlay and preserve valuation metadata, but fallback classification and downstream UI explanation are still incomplete. | Keep Hero/Reports on PRM-backed totals, then complete UI/debug explanation and cross-surface convergence. | `#395`, `#398`, `#397` | Slice 3 valuation KPIs |
| Position Value | `position_value` | Current market value of one scoped asset position. | `src/lib/calculations/global-asset-metrics.ts`; `src/lib/parqet/global-assets/aggregate.ts`; `src/lib/view-models/global-asset-view-model-builder.ts`; `src/lib/asset-detail.ts` | `partial` | Asset Table, Asset Detail, Reports | `calculations` | The default PRM valuation path now uses an explicit current market-price overlay where available, but fallback classification and full surface convergence are still pending. | Keep one canonical PRM metric, then converge remaining consumers and expose fallback/staleness explanation where needed. | `#395`, `#398`, `#397` | Slice 3 valuation KPIs |
...
| Unrealized PnL | `unrealized_pnl` | Gain/loss on the still-open position. | `src/lib/calculations/global-asset-metrics.ts`; `src/lib/parqet/global-assets/aggregate.ts`; `src/lib/view-models/global-asset-view-model-builder.ts`; `src/components/dashboard/HeroSection.tsx`; `src/lib/reporting.ts` | `partial` | Dashboard Hero, Asset Table, Asset Detail, Reports | `calculations` | PRM aggregation now derives this KPI from the corrected valuation input, but full UI convergence and richer fallback/staleness explanation are still pending. | Reuse the PRM KPI field across shared surfaces and keep improving valuation metadata for downstream explanation. | `#395`, `#398`, `#397`, `#396` | Slice 3 valuation KPIs |
