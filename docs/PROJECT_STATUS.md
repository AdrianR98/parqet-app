...
- The Phase 4 DB migration chain has been squashed into `src/lib/market-data/db/migrations/001_asset_reference_data_baseline.sql`; the active runners now execute only that current Asset/reference-data baseline.
- Intentional transition state is complete for migrations: active setup no longer replays the old `001`-`012` staged chain, and the baseline does not create legacy `market_*` tables.
- Asset-oriented repository/service functions read from `assets` + `asset_daily_prices` and derive latest market price snapshots without introducing `asset_latest_prices`.
- Phase 4 local backfill utility was used and then retired after validation: the one-shot helper `scripts/backfill-asset-reference-data.mjs` is deleted, and the validated results are retained in `docs/PHASE4_BACKFILL_VALIDATION.md`.
- The legacy JSON import helper `scripts/import-market-data-json.mjs` is also deleted; the repo no longer advertises a command that only populated the pre-Asset legacy tables.
...
- Final legacy cleanup is now superseded by the squashed baseline and is no longer part of the active migration runner path.
...
- `.DE` workflow is now explicitly two-stage: discover/validate/store `.DE` candidates first, then run the separate primary-preference command.
